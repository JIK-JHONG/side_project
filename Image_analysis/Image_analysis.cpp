...
string input_image = "birds_img.jpeg";
string option = "mix";
    ...
    if (option=="all"){
        ...
        for (int j = 0; j < input.rows; j++) {
            for (int i = 0; i < input.cols; i++) {
                intensity_r[image_intensity_r.at<uchar>(j, i)]++;  // 累計紅色通道強度
                intensity_g[image_intensity_g.at<uchar>(j, i)]++;  // 累計綠色通道強度
                intensity_b[image_intensity_b.at<uchar>(j, i)]++;  // 累計藍色通道強度
            }
        }
        intensity_r[0] = 0 ;
        intensity_b[0] = 0 ;
